...
public class ShiftDaoJPA implements ShiftDao {
    ...
    @Override
    public List<Shift> findAllShiftsByTime(LocalTime time) {
        try (Session session = sessionFactory.openSession()) {
    ...
    @Override
    public List<Shift> findShiftsByStaffIdBetweenTimes(long staffId, LocalTime startTime, LocalTime endTime) {
        try (Session session = sessionFactory.openSession()) {
